...
//		 request 객체에서 전달된 parameter를 추출한다.
		...
		int runningTime = Integer.valueOf(req.getParameter("runningTime"));
...
//			 세션에 지금까지 등록한 영화 수를 저장한다.
			HttpSession session = req.getSession();
			session.setAttribute("MovieCount", dao.MovieCount());
...
//		 이때 연결할 jsp의 이름을 넘겨준다. forward에서 /는 context root를 나타낸다.
	...
	/**
	 * 현재까지 등록한 영화 정보를 list.jsp로 forward를 통해 전달
	 * 
	 * @param req
	 * @param resp
	 * @throws ServletException
...
//		 attribute에 지금까지 등록한 영화 리스트 등록
		try {
			req.setAttribute("movies", dao.selectAllMovies());
		} catch (SQLException e) {
			e.printStackTrace();
		}
